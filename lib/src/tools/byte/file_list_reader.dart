...
class FileListReader {
  ...
  List<String> get read {
    RootDataset rds;
    final fileNoWidth = getIntWidth(paths.length);
    ...
    for (var i = 0; i < paths.length; i++) {
      ...
      if (count++ % printEvery == 0) {
        final lineNo = getPaddedInt(count, fileNoWidth);
        log.info('$lineNo Reading: $path ');
      }
      ...
      try {
        success =
            readWriteFileChecked(path, fileNumber: i, doLogging: false);
        if (success == false) {
          failures.add('"$path "');
        } else {
